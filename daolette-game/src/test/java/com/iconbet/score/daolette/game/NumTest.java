...
class NumTest {
	...
	void test() {
		...
		BigInteger payout = BigInteger.valueOf( Float.valueOf(MULTIPLIERS.get(BET_TYPES[5]) * 1000).intValue() ).multiply(amount).divide(BigInteger.valueOf(1000*numbers.size()));
		...
		assertEquals(part , Float.valueOf(MULTIPLIERS.get(BET_TYPES[5]) * 1000).intValue());
		assertEquals(BigInteger.valueOf(part) , BigInteger.valueOf( Float.valueOf(MULTIPLIERS.get(BET_TYPES[5]) * 1000).intValue()) );
		assertEquals(BigInteger.valueOf(part) , BigInteger.valueOf( (int)(MULTIPLIERS.get(BET_TYPES[5]) * 1000) ) );
